...
function gaussian:box_muller()
	local twister = self.twister
    local u = twister:random() -- 生成(0,1)之间的随机数u
    local v = twister:random() -- 生成(0,1)之间的随机数v
    local x = math.sqrt(-2 * math.log(u)) * math.cos(2 * math.pi * v)
    local y = math.sqrt(-2 * math.log(u)) * math.sin(2 * math.pi * v)
    return x, y
end

function gaussian:gaussian_random(muOne,sigmaOne,muTwo,sigmaTwo)
    local x, y = self:box_muller()
    local z = x * sigmaOne + muOne
    local w = y * sigmaTwo + muTwo
    return math_floor(z),math_floor(w)
end


function gaussian:random(min, max,sd)
  local mu = (min + max) / 2
  if not sd then
		sd = (max-min)//7
	end
	...
	error(string.format("can't find random %d,%d",min,max))
end

function gaussian:randomseed(s)
	self.twister:randomseed(s)
end

function M.gaussian(s)
	local temp = {}
	setmetatable(temp, gaussian)
	temp.twister = M.twister(s)
	return temp
end
